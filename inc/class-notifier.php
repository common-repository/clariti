...
class Notifier {
	...
	public static function action_updated_postmeta( $meta_id, $object_id, $meta_key ) {
		if ( in_array(
			...
			if ( 'wprm_recipe' === $post->post_type ) {
				...
				if ( ! $parent_post || 'publish' !== $parent_post->post_status || ! in_array( $parent_post->post_type, clariti_get_supported_post_types(), true ) ) {
					return;
				}

				self::send_event_to_clariti( $parent_post->post_type, self::UPDATE_ACTION, $parent_post_id );
	 ...
	 *
	 * @param \Tasty_Recipes\Objects\Recipe $recipe  Recipe object.
	 * @param integer                       $post_id ID for the recipe's parent post.
	 */
	public static function action_tasty_recipes_updated_recipe_rating( $recipe, $post_id ) {
		...
		if ( ! $post || 'publish' !== $post->post_status || ! in_array( $post->post_type, clariti_get_supported_post_types(), true ) ) {
			return;
		}
		...
		// Set a flag to avoid duplicate events.
		add_filter( 'clariti_sent_ratings_update_event_' . (int) $recipe->get_id(), '__return_true' );
	}
